...
nohup snap run chromium --no-sandbox --user-data-dir="${PWD}/chrome-user-data" --proxy-server=http://127.0.0.1:7891 "https://www.youtube.com/results?search_query=免费节点&sp=EgQIAhAB" > chromium.log 2>&1 &
elif [ -e /usr/bin/chromium ];then
echo "true"
nohup /usr/bin/chromium --no-sandbox --user-data-dir="${PWD}/chrome-user-data" --proxy-server=http://127.0.0.1:7891 "https://www.youtube.com/results?search_query=免费节点&sp=EgQIAhAB" > chromium.log 2>&1 &
elif [ -e /usr/bin/chromium-browser ];then
echo "true"
nohup /usr/bin/chromium-browser --no-sandbox --user-data-dir="${PWD}/chrome-user-data" --proxy-server=http://127.0.0.1:7891 "https://www.youtube.com/results?search_query=免费节点&sp=EgQIAhAB" > chromium.log 2>&1 &
elif [ -e /usr/lib64/chromium-browser/chromium-browser ];then
echo "true"
nohup /usr/lib64/chromium-browser/chromium-browser --no-sandbox --user-data-dir="${PWD}/chrome-user-data" --proxy-server=http://127.0.0.1:7891 "https://www.youtube.com/results?search_query=免费节点&sp=EgQIAhAB" > chrome.log 2>&1 &
else 
...
echo 五、脚本使用的代理配置文件为 :7894 控制界面，:7891 http/https，:7892 socks，:7893 mixed，如果你自定义修改了脚本配置文件，请以自己的配置文件为准。
