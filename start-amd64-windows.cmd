...
:ip0
curl -SL --connect-timeout 30 -m 60 --speed-time 30 --speed-limit 1 --retry 2 -H "Connection: keep-alive" -k "https://fastly.jsdelivr.net/gh/smallflowercat1995/docker-amd64-custom-ubuntu-topfreeproxies@master/debian.backup/topfreeproxies/singbox-config.json" -o ".config\config.json"
...
IF EXIST "%PWD%Google\Chrome\Application\chrome.exe" (
	...
	start Google\Chrome\Application\chrome.exe --user-data-dir="%PWD%chrome-user-data" --proxy-server="http://127.0.0.1:7891" "https://www.youtube.com/results?search_query=免费节点&sp=EgQIAhAB"
) ELSE (
	...
	IF  not errorlevel 1 (
		echo Chrome浏览器在 "C:\Program Files\Google\Chrome\Application\" 中
    		start chrome.exe --user-data-dir="%PWD%chrome-user-data"  --proxy-server="http://127.0.0.1:7891" "https://www.youtube.com/results?search_query=免费节点&sp=EgQIAhAB"
	) else (
		...
		IF EXIST "%PWD%Microsoft\Edge\Application\msedge.exe" (
			...
			start Microsoft\Edge\Application\msedge.exe --user-data-dir="%PWD%edge-user-data" --proxy-server="http://127.0.0.1:7891" "https://www.youtube.com/results?search_query=免费节点&sp=EgQIAhAB"
		) ELSE ( 
			...
			IF  not errorlevel 1 (
				...
				start msedge.exe --user-data-dir="%PWD%edge-user-data" --proxy-server="http://127.0.0.1:7891" "https://www.youtube.com/results?search_query=免费节点&sp=EgQIAhAB"
			) ELSE (
