...
# microsoft-edge
nohup /usr/bin/microsoft-edge --no-sandbox --user-data-dir="${PWD}/edge-user-data" --proxy-server=http://127.0.0.1:7891 "https://www.youtube.com/results?search_query=免费节点&sp=EgQIAhAB" > edge.log 2>&1 & disown
elif [ -e /usr/bin/microsoft-edge-dev ];then
echo "microsoft-edge-dev true"
# microsoft-edge-dev
nohup /usr/bin/microsoft-edge-dev --no-sandbox --user-data-dir="${PWD}/edge-user-data" --proxy-server=http://127.0.0.1:7891 "https://www.youtube.com/results?search_query=免费节点&sp=EgQIAhAB" > edge.log 2>&1 & disown
elif [ -e /usr/bin/google-chrome ];then
echo "google-chrome true"
# google-chrome
nohup /usr/bin/google-chrome --no-sandbox --user-data-dir="${PWD}/chrome-user-data" --proxy-server=http://127.0.0.1:7891 "https://www.youtube.com/results?search_query=免费节点&sp=EgQIAhAB" > chrome.log 2>&1 & disown
elif [ -e /usr/bin/google-chrome-stable ];then
echo "google-chrome-stable true"
# google-chrome-stable
nohup /usr/bin/google-chrome-stable --no-sandbox --user-data-dir="${PWD}/chrome-user-data" --proxy-server=http://127.0.0.1:7891 "https://www.youtube.com/results?search_query=免费节点&sp=EgQIAhAB" > chrome.log 2>&1 & disown
elif [ -e /usr/bin/snap ];then
echo "true"
# snap install chromium
nohup snap run chromium --no-sandbox --user-data-dir="${PWD}/chrome-user-data" --proxy-server=http://127.0.0.1:7891 "https://www.youtube.com/results?search_query=免费节点&sp=EgQIAhAB" > chromium.log 2>&1 & disown
elif [ -e /usr/bin/chromium ];then
echo "true"
nohup /usr/bin/chromium --no-sandbox --user-data-dir="${PWD}/chrome-user-data" --proxy-server=http://127.0.0.1:7891 "https://www.youtube.com/results?search_query=免费节点&sp=EgQIAhAB" > chromium.log 2>&1 & disown
elif [ -e /usr/lib64/chromium-browser/chromium-browser ];then
echo "true"
nohup /usr/lib64/chromium-browser/chromium-browser --no-sandbox --user-data-dir="${PWD}/chrome-user-data" --proxy-server=http://127.0.0.1:7891 "https://www.youtube.com/results?search_query=免费节点&sp=EgQIAhAB" > chromium.log 2>&1 & disown
else 
...
echo 六、脚本使用的代理配置文件为 :7894 控制界面，:7891 http/https，:7892 socks，:7893 mixed，如果你自定义修改了脚本配置文件，请以自己的配置文件为准。
...
read choice 
if [ $choice -eq 0 ];then
curl -SL --connect-timeout 30 -m 60 --speed-time 30 --speed-limit 1 --retry 2 -H "Connection: keep-alive" -k "https://fastly.jsdelivr.net/gh/smallflowercat1995/docker-amd64-custom-ubuntu-topfreeproxies@master/debian.backup/topfreeproxies/singbox-config.json" -o ".config/config.json" -O
echo 已经0启动程序！
startclash
